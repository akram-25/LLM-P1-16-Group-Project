...
def chat():
    ...
    try:
        ...
        # 1. Decide Intent
        # Pass the chat_history into the router so it understands pronouns!
        ...
        if intent == "SEARCH":
            ...
            # 1. Extract the query generated by the Router
            live_query = decision.get("query", user_input)

            # 2. Log the web search (only if they are logged in)
            if not is_guest:
                db.save_search(user_id, live_query, search_type="live_web")

            # 3. Hit DuckDuckGo instead of ChromaDB
            web_results_text = bot.search_live_web(live_query)

            # 4. Package it in the exact dictionary format your generator expects
            ...
            # 5. Generate the response using the web data
            bot_reply = bot.generate_response_with_history(
